...
public class UserRestController {
	...
	// 아이디 중복 체크
	@ResponseBody
	@PostMapping(value = "/idChk", consumes = "application/json", produces = { MediaType.TEXT_PLAIN_VALUE })
	...
	// 이메일 인증 메서드
	// @ResponseBody
	// @GetMapping(value="/emailChk",consumes="application/json",produces=
	// {MediaType.TEXT_PLAIN_VALUE})
	/*
	 * public String mailCheckGET(String email) throws Exception{
	 * System.out.println("이메일 인증 컨트롤러"); System.out.println("이메일 : " + email);
	 * 
	 * // 인증번호(난수생성) Random random = new Random(); int
	 * checkNum=random.nextInt(888888)+111111;
	 * System.out.println("인증번호 : "+checkNum);
	 * 
	 * 
	 * 이메일 보내기 String setFrom = "sos104v@naver.com"; String toMail = email; String
	 * title = "회원가입 인증 이메일 입니다."; String content = "홈페이지를 방문해주셔서 감사합니다." +
	 * "<br><br>" + "인증 번호는 " + checkNum + "입니다." + "<br>" +
	 * "해당 인증번호를 인증번호 확인란에 기입하여 주세요."; try {
	 * 
	 ...
	 * 
	 * }catch(Exception e) { e.printStackTrace(); } String num =
	 * Integer.toString(checkNum); return num; }
	 */
	 ...
	 * @ResponseBody
	 * 
	 * @PostMapping(value="/userModify" consumes="application/json",produces=
	 * {MediaType.TEXT_PLAIN_VALUE}) public String registerUpdate(ConsumerVO vo,
	 * HttpSession session) throws Exception { service.userModify(vo);
	 * session.invalidate(); return "/user/userLogin"; }
	 */
